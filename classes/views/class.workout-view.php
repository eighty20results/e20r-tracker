...
class Workout_View extends Settings_View {
	...
	public function __construct( $data = null, $error = null ) {
		...
		parent::__construct( 'workout', Workout_Model::post_type );
	...
	public function display_printable_list( $activities, $config ) {
		...
		if ( empty( $activities ) && ( 'upcoming' == $config->period ) ) { ?>
            ...
            <p><?php
				...
				Utilities::get_instance()->log( "Period setting: {$config->period}" );
				
				if ( 'current' == $config->period ) {
					$period = 'current week';
				} else if ( 'previous' == $config->period ) {
					$period = 'prior week';
				} else {
					$period = "upcoming week";
				}
				
				echo sprintf( __( 'Sorry, there are no activities to found for the %1$s.<br/>Please report this error to the %1$swebmaster%2$s', "e20r-tracker" ), $period, sprintf( '<a href="mailto:%s">', get_bloginfo( 'admin_email' ) ), '</a>' ); ?>
            </p>
            </div><?php
		}
		...
		foreach ( $activities as $id => $activity_summary ) {
			...
			foreach ( $activity_summary->days as $day ) {
				...
				$day_list[] = $Tracker->displayWeekdayName( $day );
			}
            ...
            <div class="e20r-activity-summary-table">
				...
				foreach ( $activity_summary->exercises as $id ) { ?>
					...
					$Exercise->set_currentExercise( $id );
					echo $Exercise->print_exercise( false, 'new' ); ?>
                    </div><?php
				} ?>
            </div>
			<?php
	...
	public function viewExerciseProgress( $activities = null, $error = null, $userId = null, $dimensions = null ) {
		...
		Utilities::get_instance()->log( "Listing exercise history" );
        ...
        <div class="e20r-row clear-after"> <?php
			...
			foreach ( $activities as $exId => $info ) {
				...
				Utilities::get_instance()->log( "Exercise ID: {$exId}" );
				...
				switch ( $type ) {
					...
					case 3: // AMRAP
						...
						$wType     = $Exercise->getExerciseType( $type );
	...
	public function view_WorkoutStats( $clientId = null, $exercise_id = null, $dimensions = null, $records = null ) {
		...
		$reloadBtn = sprintf( '
                    <div id="e20r_reload_btn">
                        <a href="#e20r_tracker_data" id="e20r-reload-statistics" class="e20r-choice-button button e20r-button" >%s</a>
                    </div>
                ', __( "Reload Activity Statistics", "e20r-tracker" )
		);
        ...
        <input type="hidden" name="exercise_id[]" class="e20r-workout-statistics-exercise_id"
               value="<?php esc_attr_e( $exercise_id ); ?>">
        <button
        ...
        <div class="startHidden" id="exercise_stats_<?php esc_attr_e( $exercise_id ); ?>"
	...
	public function displayArchive( $activityList, $config ) {
		...
		Utilities::get_instance()->log( "Display archive" );
		// Utilities::get_instance()->log( $activityList );
		...
		Utilities::get_instance()->log( "Check whether the archive is empty" );
		Utilities::get_instance()->log( "# of entries in archive: " . count( $activityList ) );
		...
		Utilities::get_instance()->log( "Iterating through list of activities." );
		foreach ( $activityList as $day => $activity ) { ?>
            <div id="e20r-daily-activity-page">
            <div class="e20r-faq-container e20r-toggle-close">
                <h3 class="e20r-faq-question"><?php echo $Tracker->displayWeekdayName( $day ); ?></h3>
                <div class="e20r-faq-answer-container clear-after">
	...
	public function display_printable_activity( $config, $workoutData, $hide_print = false ) {
		...
		Utilities::get_instance()->log( "Display a single activity entry for print " );
                                                ...
                                                <table class="e20r-resp-table">
													...
													for ( $i = 1; $i <= $g->group_set_count; $i ++ ) {
														...
														if ( $currentExercise->type == 2 ) {
															...
															Utilities::get_instance()->log( "Time/Reps type of exercise" );
															$weight = $currentExercise->reps;
														} else {
															Utilities::get_instance()->log( "Weight/Reps type of exercise" );
	...
	/**
	 * Generates the HTML for the activity. Typically used by the activity short code or the activity archive short
	 * code.
	 *
	...
	public function displayActivity( $config, $workoutData ) {
		...
		Utilities::get_instance()->log( "Content of workoutData object: " );
		// Utilities::get_instance()->log( $workoutData );
                    ...
                    <div class="e20r-activity-table-header">
                        <div class="e20r-exercise-row e20r-workout-summary clearfix">
                            ...
                            <div class="e20r-int-table">
                                <div class="e20r-act-content-row clearfix">
                                    <p class="e20r-content-col">
                                        <span class="e20r-exercise-label"><?php _e( "Phase", "e20r-tracker" ); ?>
                                            : </span>
                                        <span class="e20r-exercise-value"><?php echo esc_attr( $w->phase ); ?></span>
                                    </p>
                                    ...
                                    <p class="e20r-content-col right alignright">
                                        <span class="e20r-exercise-label"><?php _e( "Name", "e20r-tracker" ); ?>
                                            : </span>
                                        <span
                                                class="e20r-exercise-value"><?php echo esc_attr( $current_user->user_firstname ); ?></span>
                                    </p>
                    ...
                    <div class="e20r-activity-table-body">
						...
						foreach ( $groups as $k => $g ) {
							...
							foreach ( $g->exercises as $exKey => $exId ) {
								...
								?>
                                <div class="e20r-exercise-row fullwidth clearfix">
									<?php echo $Exercise->print_exercise( $config->expanded ); ?>
	...
	public function viewSettingsBox( $workoutData ) {
		...
		Utilities::get_instance()->log( "Loading program list for future use:" );
		
		$programs = $Program->getProgramList();
		
		Utilities::get_instance()->log( "Supplied data: " . print_r( $workoutData, true ) );
		
		if ( isset( $workoutData->days ) && empty( $workoutData->days ) ) {
			
			Utilities::get_instance()->log( "Days contains: " );
			$workoutData->days = array();
		} else if ( ! isset( $workoutData->days ) ) {
			Utilities::get_instance()->log( "No 'days' defined??" );
		}
        ...
        <div class="e20r-editform" style="width: 100%;">
            ...
            <table id="e20r-workout-settings" class="wp-list-table widefat fixed">
                ...
                <tr id="<?php echo ( isset( $workoutData->id ) ) ? $workoutData->id : $post->ID; ?>"
                    class="workout-inputs">
                    <td colspan="3">
                            ...
                            <tr>
                                <td class="select-input">
                                    <select id="e20r-workout-assigned_usergroups"
										...
										$member_groups = $Workout->getMemberGroups();
...
-->
                            ...
                            <tr>
                                <td class="text-input">
                                    <input type="number" id="e20r-workout-startday" name="e20r-workout-startday"
                                           value="<?php esc_attr_e( $workoutData->startday ); ?>">
                                </td>
                                <td class="text-input">
                                    <input type="number" id="e20r-workout-endday" name="e20r-workout-endday"
                                           value="<?php esc_attr_e( $workoutData->endday ); ?>">
                                </td>
                            </tr>
                            </tbody>
                        </table>
                ...
                <tr>
                    <td colspan="3">
                        <div id="e20r-workout-add-groups">
							<?php
							Utilities::get_instance()->log( "Loading " . count( $workoutData->groups ) . " groups of exercises: " . print_r( $workoutData->groups, true ) );
							
							foreach ( $workoutData->groups as $key => $group ) {
								
								Utilities::get_instance()->log( "Group # {$key} for workout {$workoutData->id}" );
                ...
                <tr>
                    <td colspan="3" style="text-align: right;"><a href="javascript:" class="button"
                                                                  id="e20r-new-group-button"><?php _e( 'Add Exercise Group', 'e20r-tracker' ); ?></a></td>
                </tr>
                </tfoot>
            </table>
        </div>
	...
	public function newExerciseGroup( $group = null, $group_id = null ) {
        ...
        <table class="e20r-exercise-group">
            ...
            <tr class="e20r-workout-exercise-group-header">
                <td <?php echo( $group_id == 0 ? ' colspan="2" ' : null ); ?>class="e20r-group-header">
                    <input type="hidden" name="e20r-group-id[]" value="<?php esc_attr_e( $group_id ); ?>"
                           class="e20r-group-id"/>

                    <h3><?php _e( 'Group #', 'e20r-tracker' ); ?><span
                                class="group-id"><?php esc_attr_e( $group_id + 1 ); ?></span></h3>
                </td>
				<?php if ( $group_id != 0 ) : ?>
                    <td style="text-align: right"><a href="javascript:" class="e20r-remove-group button"
                                                     id="e20r-workout-group_set_count-<?php esc_attr_e( $group_id ); ?>"><?php _e( 'Remove Group #', 'e20r-tracker' ); ?>
                            <span class="group-id"><?php esc_attr_e( $group_id + 1 ); ?></span></a></td>
				<?php endif; ?>
            </tr>
                    ...
                    <table class="sub-table wp-list-table widefat fixed">
                        ...
                        <tr>
                            <th class="e20r-label header indent"><label
                                        for="e20r-workout-group_set_count"><?php _e( 'Number of sets', 'e20r-tracker' ); ?></label>
                            </th>
                            <th class="e20r-label header" style="width: 40%;"><label
                                        for="e20r-workout-group_rests"><?php _e( 'Rest between groups (Seconds)', 'e20r-tracker' ); ?></label>
                            </th>
                            <th class="e20r-label header indent"><label
                                        for="e20r-workout-group_tempo-<?php esc_attr_e( $group_id ); ?>"><?php _e( 'Tempo', 'e20r-tracker' ); ?></label>
                            </th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            <td class="text-input">
                                <input type="number" class="e20r-workout-groups-group_set_count"
                                       ...
                                       value="<?php esc_attr_e( $group->group_set_count ); ?>" style="width: 100%;">
                            </td>
                            <td class="text-input" style="width: 40%;">
                                <input type="number" class="e20r-workout-groups-group_rest"
                                       name="e20r-workout-groups-group_rest[]"
                                       value="<?php esc_attr_e( $group->group_rest ); ?>"
                                       style="width: 100%;">
                            </td>
                            <td class="text-input">
		...
		Utilities::get_instance()->log( "Workout_View::newExerciseGroup() -- HTML generation completed." );
	...
	public function generateExerciseList( $group, $groupId ) {
        ...
        <table class="e20r-exercise-list sub-table wp-list-table widefat fixed">
            ...
            <tr>
                ...
                <th class="e20r-label header"><label
                            for="e20r-workout-exercise-reps"><?php _e( 'Reps / Duration', 'e20r-tracker' ); ?></label>
                </th>
                <th class="e20r-label header"><label
			...
			if ( count( $group->exercises ) > 0 ) {
				...
				foreach ( $group->exercises as $exId ) {
					...
					if ( $exId !== 0 ) {
						...
						$type = $Exercise->getExerciseType( $exSettings->type );
						Utilities::get_instance()->log( "Setting type to: {$type}" );
						...
						printf( "<tr>" );
						printf( '<td class="exercise-order" style="width: 15px;">%d</td>', esc_attr( $count ) );
						printf( "<td colspan='2'> %s  ( %s )</td>", esc_html( $exSettings->title ), esc_html( $exSettings->shortcode ) );
						printf( "<td>%s</td>", esc_attr( $type ) );
						printf( "<td>%d</td>", esc_attr( $exSettings->reps ) );
						printf(
							'<td> %1$s %2$s %3$s %4$s</td>',
							$exSettings->rest,
							sprintf( '<input type="hidden" class="e20r-workout-group_exercise_id" name="e20r-workout-group_exercise_id[]" value="%s" >', esc_attr( $exSettings->id ) ),
							sprintf( '<input type="hidden" class="e20r-workout-group_exercise_order" name="e20r-workout-group_exercise_order[]" value="%d" >', esc_attr( $count ) ),
							sprintf( '<input type="hidden" class="e20r-workout-group" name="e20r-workout-group[]" value="%d" >', esc_attr( $groupId ) )
						);
						
						printf(
							'<td><a href="javascript:e20rActivity.editExercise( \'group:%d\', %d, %d);" class="e20r-exercise-edit">%s</a></td>',
							esc_attr( $groupId ),
							esc_attr( $exSettings->id ),
							esc_attr( $count ),
							__( "Update", "e20r-tracker" )
						);
						
						printf(
							'<td><a href="javascript:e20rActivity.removeExercise( \'group:%d\', %d, %d);" class="e20r-exercise-remove">%s</a></td>',
							esc_attr( $groupId ),
							esc_attr( $exSettings->id ),
							esc_attr( $count ),
							__( "Remove", "e20r-tracker" )
						);
						printf( "</tr>" );
            ...
            <table class="new-exercises" style="width: 100%;">
                ...
                <tr>
                    ...
                    <th class="new-exercise-header-exercise-id"><label
                                for="e20r-workout-add-exercise-id"><?php _e( 'Exercise', 'e20r-tracker' ); ?></label>
                    </th>
                    <th></th>
                </tr>
                </thead>
                ...
                <tr>
                    ...
                    <td style="width: 70%">
                        <select class="e20r-select2-container select2 e20r-workout-add-exercise-id"
							...
							Utilities::get_instance()->log( "Loading all possible exercises" );
							
							$all = $Exercise->getAllExercises();
							
							Utilities::get_instance()->log( "Loaded " . count( $all ) . ' exercises' );
							
							foreach ( $all as $exercise ) {
								printf(
									'<option value="%d">%s (%s)</option>',
									esc_attr( $exercise->id ),
									esc_attr( $exercise->title ),
									esc_attr( $exercise->shortcode )
								);
							} ?>
                        </select>
                    </td>
